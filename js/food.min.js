...
function reInitChefTable(e) {
    ...
    o.MakeCellsEditable({
        ...
        onUpdate: function(a, t, i, l) {
            ...
            // 编辑已修炼列（31）或已有列（32）时，更新ForRestore值
            if (i.index().column == 31 && i.data() != l) {
                var chefData = t.data();
                chefData.initialUltimateForRestore = i.data();
                chefData.originalUltimate = i.data();
            }
            if (i.index().column == 32 && i.data() != l) {
                var chefData = t.data();
                chefData.initialGotForRestore = i.data();
                chefData.originalGot = i.data();
            }
            
            // 编辑已修炼列时，同步更新计算器配置页面的下拉框
            if (i.index().column == 31 && i.data() != l) {
                var chefData = t.data();
                var chefId = String(chefData.chefId); // 转换为字符串以确保类型一致
                var chefName = chefData.name;
                var isUltimated = (i.data() == "是");
                
                // 更新上场类已修炼下拉框 (chk-cal-partial-ultimate)
                if (chefData.ultimateSkillEffect) {
                    var hasPartialSkill = false;
                    var partialSkillDisp = chefData.ultimateSkillDisp;
                    
                    for (var usi in chefData.ultimateSkillEffect) {
                        if (("Partial" == chefData.ultimateSkillEffect[usi].condition || "Next" == chefData.ultimateSkillEffect[usi].condition) &&
                            "OpenTime" != chefData.ultimateSkillEffect[usi].type && 
                            "CookbookTime" != chefData.ultimateSkillEffect[usi].type) {
                    ...
                    if (hasPartialSkill) {
                        var $partialSelect = $("#chk-cal-partial-ultimate");
                        var $existingOption = $partialSelect.find('option[value="' + chefId + '"]');
                        
                        if (isUltimated) {
                            // 改为"是"：确保选项存在并勾选
                            if ($existingOption.length == 0) {
                                // 选项不存在，添加选项
                                var optionHtml = "<option value='" + chefId + "' data-subtext='" + partialSkillDisp.replace(/<br>/g, " ") + "'>" + chefName + "</option>";
                                $partialSelect.append(optionHtml);
                                $partialSelect.selectpicker('refresh');
                            }
                            
                            // 自动勾选该厨师
                            var currentVal = $partialSelect.val() || [];
                            if (currentVal.indexOf(chefId) === -1) {
                                currentVal.push(chefId);
                                $partialSelect.selectpicker('val', currentVal);
                            }
                        } else if (!isUltimated) {
                            // 改为"否"：取消勾选
                            var currentVal = $partialSelect.val() || [];
                            var index = currentVal.indexOf(chefId);
                            if (index !== -1) {
                ...
                // 更新个人类已修炼下拉框 (chk-cal-self-ultimate)
                if (chefData.ultimateSkillEffect) {
                    var hasSelfSkill = false;
                    var selfSkillDisp = chefData.ultimateSkillDisp;
                    
                    for (var usi2 in chefData.ultimateSkillEffect) {
                        if ("Self" == chefData.ultimateSkillEffect[usi2].condition && 
                            "Material_Gain" != chefData.ultimateSkillEffect[usi2].type) {
                            hasSelfSkill = true;
                            break;
                        }
                    }
                    ...
                    if (hasSelfSkill) {
                        var $selfSelect = $("#chk-cal-self-ultimate");
                        var $existingOption2 = $selfSelect.find('option[value="' + chefId + '"]');
                        
                        if (isUltimated) {
                            // 改为"是"：确保选项存在并勾选
                            if ($existingOption2.length == 0) {
                                // 选项不存在，添加选项
                                var optionHtml2 = "<option value='" + chefId + "' data-subtext='" + selfSkillDisp.replace(/<br>/g, " ") + "'>" + chefName + "</option>";
                                $selfSelect.append(optionHtml2);
                                $selfSelect.selectpicker('refresh');
                            }
                            
                            // 自动勾选该厨师
                            var currentVal2 = $selfSelect.val() || [];
                            if (currentVal2.indexOf(chefId) === -1) {
                                currentVal2.push(chefId);
                                $selfSelect.selectpicker('val', currentVal2);
                            }
                        } else if (!isUltimated) {
                            // 改为"否"：取消勾选
                            var currentVal2 = $selfSelect.val() || [];
                            var index2 = currentVal2.indexOf(chefId);
                            if (index2 !== -1) {
                                currentVal2.splice(index2, 1);
                                $selfSelect.selectpicker('val', currentVal2);
                            }
                        }
                    }
...
function getCustomRecipeIdByName(e, a) {
    ...
    var foundId = null;
    for (var t in e)
        if (e[t].recipe && e[t].recipe.data) {
            var menuName = $.trim(e[t].recipe.data.name || "");
            // 精确相等或前缀/后缀相等都认为是同一道菜
            if (menuName === a || a.indexOf(menuName) === 0 || menuName.indexOf(a) === 0) {
                foundId = e[t].recipe.data.recipeId;
                break;
            }
        }
    return foundId
...
// 公共函数：判断是否为贵客率计算模式
// 返回：true 表示贵客率计算模式，false 表示其他模式
function isGuestRateMode() {
    return calCustomRule && calCustomRule.isGuestRate === true;
}

// 公共函数：检测已上场菜谱的贵客冲突
// 参数：ruleIndex - 规则索引
function checkGuestConflictsForSelectedRecipes(ruleIndex) {
    if (!isGuestRateMode()) {
        return [];
    }
    
    var gameData = calCustomRule.gameData || null;
    ...
    return conflictInfo;  // 返回冲突信息
}

// 公共函数：获取配置页面中勾选的已修炼厨师ID列表
function getConfigUltimatedChefIds() {
    ...
    return {
        ...
        all: allIds,
        allSet: allIdsSet  // 新增：用于快速查找的对象
    };
}

// 公共函数：检查厨师是否已修炼
// 参数：chefId - 厨师ID, localData - 本地数据（可选）, configIds - 配置ID缓存（可选，用于性能优化）
...
function generateChefsExportData() {
    ...
    for (var t in a) {
        var i = {};
        i.id = a[t].chefId,
        // 保存initialGotForRestore和initialUltimateForRestore而不是当前的got和ultimate
        i.got = a[t].initialGotForRestore || a[t].got,
        i.ult = a[t].initialUltimateForRestore || a[t].ultimate,
...
function initCalTables(e, a) {
    ...
    // 初始化贵客率计算器交互
    initGuestRateCalculator();
}

function initGuestRateCalculator() {
    // 初始化selectpicker
    ...
    $("#guest-rate-input").on("input change", function() {
        ...
        // 贵客率输入框变化不需要重新计算，因为贵客率是从厨师技能计算出来的
    });
    ...
    // 份数增减按钮
    $("#quantity-minus-2").click(function() {
        var input = $("#quantity-value-2");
        var val = parseInt(input.val()) || 7;
        if (val > 1) {
            input.val(val - 1).trigger("change");
        }
    });
    
    $("#quantity-plus-2").click(function() {
        var input = $("#quantity-value-2");
        var val = parseInt(input.val()) || 7;
        input.val(val + 1).trigger("change");
    });
    
    $("#quantity-minus-3").click(function() {
        var input = $("#quantity-value-3");
        var val = parseInt(input.val()) || 7;
        if (val > 1) {
            input.val(val - 1).trigger("change");
    ...
    $("#quantity-plus-3").click(function() {
        var input = $("#quantity-value-3");
        var val = parseInt(input.val()) || 7;
        input.val(val + 1).trigger("change");
    });
    ...
    // 初始化 selectpicker
    $("#star-level-2, #star-level-3").selectpicker();
    
    // 初始计算一次（不需要调用，因为会在加载规则时自动计算）
}

// 贵客率计算器的计算函数
function calculateGuestRateResults() {
    // 获取输入值
    ...
    $("#time-percentage").text("100%"); // 时间加成暂时显示100%
...
function calculateActualGuestRate(starLevel, quantity, baseGuestRate) {
    // 检查最低份数要求
    var minQuantity = {1: 20, 2: 15, 3: 12, 4: 10, 5: 7}[starLevel] || 7;
...
function getRuneRateByRank(rank) {
    ...
    // TODO: 后续需要根据厨师技能计算特殊加成
    // 检查是否有"场上厨师制作特级及以上料理时此料理贵客符文概率+X%"技能
...
function loadHistoryRules(e) {
    if (historyRulesData && historyRulesData.length > 0) {
        ...
        var firstChefCompetitionIndex = -1; // 记录第一个厨神大赛规则的索引
        ...
        for (var i = 0; i < historyRulesData.length; i++) {
            ...
            if (historyRulesData[i].tag.indexOf("大赛") >= 0) {
                ...
                // 记录第一个厨神大赛规则的索引
                if (firstChefCompetitionIndex === -1) {
                    firstChefCompetitionIndex = i;
                }
            } else {
        ...
        // 自动选中第一个厨神大赛规则并加载其详情
        if (firstChefCompetitionIndex !== -1) {
            $("#select-cal-rule").val("history_" + firstChefCompetitionIndex).selectpicker("refresh");
            var firstStartTime = historyRulesData[firstChefCompetitionIndex].start_time;
...
function initCalRulesLocal(e, a) {
    ...
    $("#chk-cal-partial-ultimate").selectpicker().on("changed.bs.select", function() {
        // 用户手动修改了选择框，需要重新计算 ultimateData
...
function setCalConfigData(e) {
    // 不重新计算 ultimateData，使用 loadUltimate 已经设置好的数据
    // 这样可以保留"读取全修炼"或"读取已修炼"时的七侠加成状态
...
function calCustomResults(e) {
    for (var a = 0, t = 0; t < calCustomRule.rules.length; t++) {
        ...
        for (var c in l) {
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    ...
                    // 不自动调整quantity，保持用户设置的值
                    // 只确保quantity不超过max
                    if (l[c].recipes[r].quantity > recipeMax) {
        ...
        for (var c in l)
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    ...
                    w.find("input").attr("max", D).val(actualQuantity),
                    w.find(".max").html(D),
        ...
        // 贵客率模式：显示贵客冲突提示
        if (isGuestRateMode()) {
            var conflictInfo = checkGuestConflictsForSelectedRecipes(t);
            if (conflictInfo && conflictInfo.length > 0) {
                ...
                conflictHtml += "</span>";
                conflictHtml += "</div>";
                $(".cal-custom-item:eq(" + t + ") .selected-sum").append(conflictHtml);
            }
        }
    }
...
// 计算贵客率计算器的字段（贵客率、暴击率、时间等）
function calculateGuestRateFields(custom, rule, starLevel, quantity, qualityLevel) {
    var result = {
        guestRate: 0,      // 贵客率
    ...
    var configUltimatedIds = getConfigUltimatedChefIds();
        ...
        if (chef.disk && chef.disk.ambers) {
            for (var a = 0; a < chef.disk.ambers.length; a++) {
                ...
                if (amber.data && amber.data.allEffect && chef.disk.level) {
                    ...
                    var amberDesc = getAmberDisplayByLevel(amber.data, chef.disk.level);
                    if (levelEffect) {
    ...
    // ========== 计算符文率 ==========
    // 品级对应的默认符文率：可50%、优51%、特52%、神53%、传53%
    var qualityLevelMap = {
        "1": 50,  // 可
        "2": 51,  // 优
        "3": 52,  // 特
    ...
    // 获取品级对应的默认符文率
    var baseRuneRate = qualityLevelMap[qualityLevel] || 50;
    ...
    if (starLevel && quantity) {
        ...
        var minQuantity = minQuantityMap[starLevel] || 7;
    ...
    // ========== 计算百锅产出 ==========
    // 公式：0.01 * 实际贵客率 * 符文率 * 暴击率
    // 先对每个值进行截断取整保留两位小数，然后再计算
    if (result.actualGuestRate !== undefined && result.runeRate !== undefined && result.critRate !== undefined) {
        ...
        // ========== 计算单位食材 ==========
        // 公式：0.1 / 份数 * 实际贵客率 * 符文率 * 暴击率
        // 使用截断后的值计算，保留两位小数直接截断，最后除以100
...
// 计算同贵客双菜谱的数据
function calculateDualRecipeData() {
    // 获取输入值
    var baseGuestRate = parseFloat($("#guest-rate-input").val()) || 0;
    ...
    // 内部函数：计算实际贵客率
    function calculateActualGuestRateInternal(starLevel, quantity, baseGuestRate) {
        // 检查最低份数要求
        var minQuantity = {1: 20, 2: 15, 3: 12, 4: 10, 5: 7}[starLevel] || 7;
        ...
        // 截断取整后除以100
        return Math.floor(actualRate * 100) / 100;
    }
    
    // 计算实际贵客率1
    var actualGuestRate1 = calculateActualGuestRateInternal(starLevel1, quantity1, baseGuestRate);
    
    // 计算实际贵客率2
    var actualGuestRate2 = calculateActualGuestRateInternal(starLevel2, quantity2, baseGuestRate);
...
function updateCalSummaryDisplay(calCustomRule) {
    ...
    // 计算并更新贵客率计算器的所有字段
    if (isGuestRateMode() && calCustomRule.rules.length > 0) {
        // 获取星级、份数和品级
        var starLevel = parseInt($("#star-level").val()) || 5;
        var quantity = parseInt($("#quantity-value").val()) || 7;
        var qualityLevel = $("#quality-level").val() || "1";
        
        var fields = calculateGuestRateFields(calCustomRule.rules[0].custom, calCustomRule.rules[0], starLevel, quantity, qualityLevel);
...
function getCustomChefsOptions(e, a, t) {
    ...
    // 判断是否为贵客率计算模式（使用公共函数）
    var isGuestRateModeFlag = isGuestRateMode();
    ...
    // 获取七侠加成数据
    var qixiaData = r.calQixiaData || null;
    
    // 获取本地数据，用于检查厨师修炼状态
    var localData = getLocalData();
    
    // 统一的厨师技能分析函数
    // 返回: { hasSkills: boolean, categories: string, skillValues: object }
    function analyzeChefGuestRateSkills(chef, qixiaData, useEquip, useAmber, localData, configUltimatedIds) {
        var result = {
            hasSkills: false,
            categories: '',
            skillValues: {
        ...
        // 检查该厨师是否已修炼（使用公共函数，传入缓存的 configUltimatedIds）
        var isUltimated = isChefUltimated(chef.chefId, localData, configUltimatedIds);
        
        // 收集所有技能来源（带来源信息，用于计算技能值）
        var allSkillSources = [];
        ...
        if (chef.ultimateSkillEffect) {
            for (var i = 0; i < chef.ultimateSkillEffect.length; i++) {
                ...
                if (skill.type === 'GuestApearRate') {
                    ...
                    categoryFlags.crit = true;
                } else if (skill.type === 'OpenTime' || skill.type === 'CookbookTime') {
                    categoryFlags.time = true;
                } else if (skill.type === 'GuestAntiqueDropRate') {
                    categoryFlags.rune = true;
                }
        ...
        for (var i = 0; i < allSkillSources.length; i++) {
            ...
            if (skill.type === 'GuestApearRate') {
                ...
                if (skill.conditionType === 'PerRank') {
                    // PerRank 类型：默认乘以 3（假设有3个符合条件的菜谱）
                    var percentMatch = desc.match(/(\d+)%/);
                    if (percentMatch) {
        ...
        if (categoryFlags.time) {
            ...
            // 计算修炼技能的时间加成（只有已修炼时才统计）
            if (isUltimated && chef.ultimateSkillEffect) {
                totalTimeAddition += getTimeAddition(chef.ultimateSkillEffect);
            }
            
            // 计算厨具技能的时间加成（只有勾选"已配厨具"时才统计）
            if (useEquip && chef.equip && chef.equip.effect) {
                // 处理MutiEquipmentSkill类技能（使厨具技能效果翻倍）
                var equipEffect = updateEquipmentEffect(chef.equip.effect, chef.selfUltimateEffect || []);
                totalTimeAddition += getTimeAddition(equipEffect);
            }
            
            // 心法盘技能不包含OpenTime，所以不需要计算
            
            // 保存原始的时间加成值（负数表示减少时间，正数表示增加时间）
            // 与额外使用上场类技能保持一致的排序逻辑
    ...
    for (var h in f) {
        ...
        if (!i || isOwned) {
            ...
            // 贵客率计算模式下，分析厨师技能（但不过滤）
            var chefAnalysis = null;
            if (isGuestRateModeFlag) {
                // 传入缓存的 configUltimatedIds
                chefAnalysis = analyzeChefGuestRateSkills(f[h], qixiaData, l, s, localData, configUltimatedIds);
            }
            ...
            // 贵客率计算模式：跳过已选择的厨师，不添加到列表中
            if (isGuestRateModeFlag && m.disabled) {
                continue;
            }
            
            // 贵客率计算模式：只为可用的厨师添加分类标记和技能值
            if (isGuestRateModeFlag && chefAnalysis) {
                m.category = chefAnalysis.categories;
...
function getCustomEquipsOptions(e, a, t, i) {
    ...
    // 统一的厨具技能分析函数
    function analyzeEquipGuestRateSkills(equip) {
        var result = {
            hasSkills: false,
            categories: '',
            skillValues: {
                guestRate: 0,
                time: 0,
                rarity: equip.rarity || 0
            }
        };
        
        var targetSkillTypes = ['GuestApearRate', 'OpenTime'];
        var categoryFlags = {
            guestRate: false,
            time: false
        };
    ...
    for (var u in t) {
        ...
        if (isGuestRateModeFlag) {
            equipAnalysis = analyzeEquipGuestRateSkills(t[u]);
        }
        ...
        // 贵客率计算模式：添加分类标记和技能值
        if (isGuestRateModeFlag && equipAnalysis) {
            h.category = equipAnalysis.categories,
...
function getCustomRecipesOptions(e, a, t, i) {
    ...
    for (var y in k) {
        ...
        if ((!l || x.data.got) && !(s.length > 0 && s.indexOf(x.data.rarity.toString()) < 0)) {
            ...
            if (!(n.length > 0 && n.indexOf(x.data.condiment) < 0)) {
                ...
                // 在贵客率模式下，需要检查所有位置（包括当前位置）
                if (isGuestRateModeFlag) {
                    for (var G in calCustomRule.rules) {
                        ...
                        for (var v in P) {
                            for (var b in P[v].recipes) {
                                if (P[v].recipes[b].data && P[v].recipes[b].data.recipeId == x.data.recipeId) {
                                    // 在贵客率模式下，任何位置的菜谱都标记为已选择（包括当前位置）
                                    isRecipeSelected = true;
                                    break;
                                }
                            }
                            if (isRecipeSelected) break;
                        }
                        if (isRecipeSelected) break;
                    }
                } else {
                    ...
                    calCustomRule.rules) {
                        ...
                        for (var v in P) {
                            for (var b in P[v].recipes) {
                                if (P[v].recipes[b].data && P[v].recipes[b].data.recipeId == x.data.recipeId) {
                                    // 判断是否是当前位置
                                    var isCurrentPosition = (G == e && v == a && b == t);
                ...
                // 贵客率计算模式：跳过已选择的菜谱，不添加到列表中
                if (isGuestRateModeFlag && isRecipeSelected) {
                    continue;
                }
                
                // 贵客率计算模式：添加贵客信息和符文信息
                var guestNames = [];
                var runeNameForGuest = null;  // 用于保存符文名称
                if (isGuestRateModeFlag && i && i.guests) {
                    i.guests.forEach(function(guest) {
                        if (guest.gifts) {
                            guest.gifts.forEach(function(gift) {
                                if (gift.recipe === x.data.name) {
                                    // 保存符文名称（如果有）
                                    if (gift.antique && !runeNameForGuest) {
                                        runeNameForGuest = gift.antique;
                                    }
                                    // 避免重复添加
                ...
                if (guestNames.length > 0) {
                    if (runeNameForGuest) {
                        w.guestNames = runeNameForGuest + '-' + guestNames.join('、');
                    } else {
                        w.guestNames = guestNames.join('、');
                    }
                }
                ...
                if (u.hasOwnProperty("MaterialsNum")) {
                    ...
                    // 贵客率计算模式：默认显示单份时间并按时间升序排序
                    if (isGuestRateModeFlag) {
                        w.content += "<span class='score'>" + x.data.timeDisp + "</span>";
                        w.order = x.data.time;  // 按单份时间排序（升序）
                    } else if ("分数" == o) {
                ...
                // 贵客率计算模式：添加符文分类标记
                // 需要从贵客的gifts数据中查找该菜谱对应的符文
                if (isGuestRateModeFlag) {
                    // 定义金符文、银符文和铜符文列表
                    var goldRunes = ['恐怖利刃', '鼓风机', '蒸馏杯', '千年煮鳖', '香烤鱼排', '五星炒果'];
                    var silverRunes = ['刀嘴鹦鹉', '一昧真火', '蒸汽宝石', '耐煮的水草', '焦虫', '暖石'];
                    var bronzeRunes = ['剪刀蟹', '油火虫', '蒸汽耳环', '防水的柠檬', '烤焦的菊花', '五香果'];
                        ...
                        for (var guestIdx = 0; guestIdx < i.guests.length; guestIdx++) {
                            ...
                            if (guest.gifts) {
                                for (var giftIdx = 0; giftIdx < guest.gifts.length; giftIdx++) {
                                    ...
                                    if (gift.recipe === x.data.name && gift.antique) {
                                        runeFound = gift.antique;
                                        break;
                                    }
                                }
                                if (runeFound) break;
                    ...
                    // 如果找到符文，判断分类并保存符文名称
                    if (runeFound) {
                        w.runeName = runeFound;  // 保存符文名称
                        
                        if (goldRunes.indexOf(runeFound) >= 0) {
                            w.category = 'gold-rune-category';
                            w.runeType = 'gold';
                        } else if (silverRunes.indexOf(runeFound) >= 0) {
                            w.category = 'silver-rune-category';
                            w.runeType = 'silver';
                        } else if (bronzeRunes.indexOf(runeFound) >= 0) {
                            w.category = 'bronze-rune-category';
                            w.runeType = 'bronze';
                        }
                    }
                }
    ...
    // 贵客率计算模式：按时间升序排序（时间短的在前）
    // 非贵客率模式：按order降序排序（分数高的在前）
    if (isGuestRateModeFlag) {
        g.sort(function(e, a) {
            return e.order - a.order;  // 升序：时间短的在前
        });
    } else {
        g.sort(function(e, a) {
            return a.order - e.order;  // 降序：分数高的在前
        });
    }
    w = {
        display: "无菜谱",
...
function initCalCustomTable(e) {
    ...
    $(".chef-box .chef-placeholder, .chef-box .chef-box-1").click(function() {
        ...
        var $select = $(this).closest(".chef-box").find("select.select-picker-chef");
        $select.html(getOptionsString(i)).selectpicker("refresh");
        // 清除数据同步标记，因为 HTML 已更新
        $select.parent().find('.dropdown-menu ul.inner').removeData('chef-data-synced');
    ...
    $("select.select-picker-chef").on("shown.bs.select", function() {
        ...
        if (!isGuestRateMode()) {
            // 非贵客率模式：移除可能残留的分类标签
            $(this).parent().find('.dropdown-menu .chef-category-tabs').remove();
            return;
        }
        
        var $select = $(this);
        // 使用 bootstrap-select 实例引用，避免取错 dropdown（尤其在页面有多个 .dropdown-menu 时）
        var sp = $select.data('selectpicker');
        var $dropdown = sp && sp.$menu ? sp.$menu : $select.parent().find('.dropdown-menu');
        var $innerMenu = sp && sp.$menuInner ? sp.$menuInner : $dropdown.find('ul.inner');
        
        // 下拉菜单可能会被 selectpicker 重建：分类/排序逻辑改为基于 option（数据源），并让 selectpicker 重建列表
        // 记录初始顺序（仅需一次；当 options 重置时会重新生成，需重新标记）
        ...
        // 创建分类标签（如果不存在）
        if (!$dropdown.find('.chef-category-tabs').length) {
            var tabsHtml = '<ul class="nav nav-tabs chef-category-tabs" style="margin: 5px 10px 5px 10px; border-bottom: 1px solid #ddd; list-style: none; padding: 0; display: flex; width: calc(100% - 20px);">' +
                '<li class="active" style="flex: 1; text-align: center; margin: 0;"><a href="#" class="chef-tab-all" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">全部</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="chef-tab-guest" data-category="guest-rate-category" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">贵客</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="chef-tab-crit" data-category="crit-category" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">暴击</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="chef-tab-time" data-category="time-category" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">时间</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="chef-tab-rune" data-category="rune-category" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">符文</a></li>' +
                '</ul>';
        ...
        // 通用的过滤和排序函数（基于 option，避免 dropdown 重建导致分类/排序丢失）
        function filterChefs(categoryName, sortKey) {
            // 保存当前选中的分类到全局变量（所有选择框共享）
            globalChefCategory = {category: categoryName, sortKey: sortKey};
            
            sp = $select.data('selectpicker') || sp;
            if (!sp) {
                return;
            }
            
            // 1) 分类过滤：通过隐藏 option，让 selectpicker 只渲染当前分类
            $select.find('option').each(function() {
                var $opt = $(this);
                var cat = $opt.attr('data-category') || '';
                // 保留空/占位项（没有分类的不参与过滤）
                if ($opt.val() === "" || $opt.val() === null || $opt.text().trim() === "") {
                    $opt.prop('hidden', false).removeAttr('data-hidden');
                    return;
                }
                ...
                $opt.prop('hidden', !shouldShow);
                if (shouldShow) {
                    $opt.removeAttr('data-hidden');
                } else {
                    $opt.attr('data-hidden', 'true');
                }
            });
            
            // 2) 排序：对“可见且非占位”的 option 进行排序，再把 DOM 重新挂回
            if (sortKey) {
                ...
                $options.each(function() {
                    ...
                    var isHidden = $opt.prop('hidden') === true || $opt.attr('data-hidden') === 'true';
                ...
                // hidden 项保持原始顺序，避免抖动
                hiddens.sort(function(a, b) {
                    return Number(a.attr('data-orig-order') || 0) - Number(b.attr('data-orig-order') || 0);
                });
                
                $select.empty();
                for (var i = 0; i < placeholders.length; i++) $select.append(placeholders[i]);
                for (var j = 0; j < visibles.length; j++) $select.append(visibles[j]);
                for (var k = 0; k < hiddens.length; k++) $select.append(hiddens[k]);
            } else {
                // 不排序：全部分类时恢复原始顺序
                if (!categoryName) {
            ...
            // 3) 让 selectpicker 基于最新 option 重新 buildData + 重建列表
            // 注意：仅 render/buildList 不会重建 main.data，导致“分类筛选失效仍展示全部”
            sp.refresh();
        }
        
        // 绑定标签点击事件
        $dropdown.find('.chef-tab-all, .chef-tab-guest, .chef-tab-crit, .chef-tab-time, .chef-tab-rune').off('click').on('click', function(e) {
            e.stopPropagation();
            e.preventDefault();
            ...
            // 根据标签确定排序键
            if ($tab.hasClass('chef-tab-all')) {
                sortKey = null;  // 全部标签：不排序
            } else if ($tab.hasClass('chef-tab-guest')) {
                sortKey = 'guestRate';  // 贵客标签：按 GuestApearRate 排序
            } else if ($tab.hasClass('chef-tab-crit')) {
                sortKey = 'crit';  // 暴击标签：按 GuestDropCount 排序
            } else if ($tab.hasClass('chef-tab-time')) {
                sortKey = 'time';  // 时间标签：按 OpenTime 排序
            } else if ($tab.hasClass('chef-tab-rune')) {
                sortKey = 'rune';  // 符文标签：按 GuestAntiqueDropRate 排序
            }
            
            // 切分类：先按分类/排序重建列表，再让搜索在当前分类结果里生效
            var searchText = ($dropdown.find('.bs-searchbox input').val() || '').trim();
            filterChefs(categoryName === 'all' ? null : categoryName, sortKey);
            // refresh 会重建 searchbox，需要重新获取后再触发搜索
            var $newSearchInput = $dropdown.find('.bs-searchbox input');
            if (searchText && $newSearchInput.length > 0) {
                $newSearchInput.val(searchText).trigger('input').trigger('keyup');
            }
        ...
        if (categoryToApply) {
            // 恢复标签选中状态
            $dropdown.find('.chef-category-tabs li').removeClass('active');
            
            if (categoryToApply.category === null) {
                // 显示"全部"标签
                $dropdown.find('.chef-tab-all').parent().addClass('active');
            } else {
                // 显示对应的分类标签
                var $targetTab = $dropdown.find('.chef-category-tabs a[data-category="' + categoryToApply.category + '"]');
                if ($targetTab.length) {
            ...
            filterChefs(categoryToApply.category, categoryToApply.sortKey);
        } else {
            // 如果没有保存的分类，默认显示全部
            $dropdown.find('.chef-tab-all').parent().addClass('active');
            filterChefs(null, null);
        }
    }),
    ...
    $(".equip-box").click(function() {
        ...
        var $select = $(this).find("select.select-picker-equip");
        $select.html(getOptionsString(l)).selectpicker("refresh");
        // 清除数据同步标记，因为 HTML 已更新
        $select.parent().find('.dropdown-menu ul.inner').removeData('equip-data-synced');
        $select.selectpicker("toggle");
    }),
    $("select.select-picker-equip").on("shown.bs.select", function() {
        ...
        if (!isGuestRateMode()) {
            // 非贵客率模式：移除可能残留的分类标签
            $(this).parent().find('.dropdown-menu .equip-category-tabs').remove();
            return;
        }
        ...
        // 创建分类标签（如果不存在）
        if (!$dropdown.find('.equip-category-tabs').length) {
            var tabsHtml = '<ul class="nav nav-tabs equip-category-tabs" style="margin: 5px 10px 5px 10px; border-bottom: 1px solid #ddd; list-style: none; padding: 0; display: flex; width: calc(100% - 20px);">' +
                '<li class="active" style="flex: 1; text-align: center; margin: 0;"><a href="#" class="equip-tab-all" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">全部</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="equip-tab-guest" data-category="guest-rate-category" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">贵客</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="equip-tab-time" data-category="time-category" style="padding: 5px 8px; display: block; text-decoration: none; white-space: nowrap; font-size: 13px;">时间</a></li>' +
                '</ul>';
        ...
        // 通用的过滤和排序函数（基于 option，避免 dropdown 重建导致分类/排序丢失）
        function filterEquips(categoryName, sortKey) {
            // 保存当前选中的分类到全局变量（所有选择框共享）
        ...
        // 绑定标签点击事件
        $dropdown.find('.equip-tab-all, .equip-tab-guest, .equip-tab-time').off('click').on('click', function(e) {
            e.stopPropagation();
            e.preventDefault();
            $(this).parent().addClass('active').siblings().removeClass('active');
            ...
            // 根据标签确定排序键
            if ($tab.hasClass('equip-tab-all')) {
                sortKey = null;  // 全部标签：不排序
            } else if ($tab.hasClass('equip-tab-guest')) {
                sortKey = 'guestRate';  // 贵客标签：按 GuestApearRate 排序
            } else if ($tab.hasClass('equip-tab-time')) {
                sortKey = 'time';  // 时间标签：按 OpenTime 排序
            }
            
            // 切分类：先按分类/排序重建列表，再让搜索在当前分类结果里生效
            var searchText = ($dropdown.find('.bs-searchbox input').val() || '').trim();
            filterEquips(categoryName === 'all' ? null : categoryName, sortKey);
            // refresh 会重建 searchbox，需要重新获取后再触发搜索
            var $newSearchInput = $dropdown.find('.bs-searchbox input');
            if (searchText && $newSearchInput.length > 0) {
                $newSearchInput.val(searchText).trigger('input').trigger('keyup');
        ...
        // 立即应用保存的分类，避免闪动
        var categoryToApply = globalEquipCategory;
        
        if (categoryToApply) {
            // 恢复标签选中状态
            $dropdown.find('.equip-category-tabs li').removeClass('active');
            
            if (categoryToApply.category === null) {
                // 显示"全部"标签
                $dropdown.find('.equip-tab-all').parent().addClass('active');
            } else {
                // 显示对应的分类标签
                var $targetTab = $dropdown.find('.equip-category-tabs a[data-category="' + categoryToApply.category + '"]');
                if ($targetTab.length) {
                    $targetTab.parent().addClass('active');
                } else {
                    // 如果没有找到对应的标签，显示全部
            ...
            filterEquips(categoryToApply.category, categoryToApply.sortKey);
        } else {
            // 如果没有保存的分类，默认显示全部
            $dropdown.find('.equip-tab-all').parent().addClass('active');
            filterEquips(null, null);
        }
    }),
    ...
    $(".recipe-box .recipe-placeholder, .recipe-box .content, .recipe-box .recipe-result, .recipe-box .recipe-box-2").click(function() {
        ...
        var $select = $(this).closest(".recipe-box").find("select.select-picker-recipe");
        $select.html(getOptionsString(l)).selectpicker("refresh");
        // 清除数据同步标记，因为 HTML 已更新
        $select.parent().find('.dropdown-menu ul.inner').removeData('recipe-data-synced');
        $select.selectpicker("toggle");
    }),
    $("select.select-picker-recipe").on("shown.bs.select", function() {
        ...
        if (!isGuestRateMode()) {
            // 非贵客率模式：移除可能残留的分类标签
            $(this).parent().find('.dropdown-menu .recipe-category-tabs').remove();
            $(this).parent().find('.dropdown-menu li.rune-group-header').remove();
            return;
        }
        ...
        // 创建分类标签（如果不存在）
        if (!$dropdown.find('.recipe-category-tabs').length) {
            var tabsHtml = '<ul class="nav nav-tabs recipe-category-tabs" style="margin: 10px 10px 10px 10px; border-bottom: 2px solid #ddd; list-style: none; padding: 0; display: flex; width: calc(100% - 20px);">' +
                '<li class="active" style="flex: 1; text-align: center; margin: 0;"><a href="#" class="recipe-tab-all" style="padding: 8px 15px; display: block; text-decoration: none; white-space: nowrap;">全部</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="recipe-tab-gold" data-category="gold-rune-category" style="padding: 8px 15px; display: block; text-decoration: none; white-space: nowrap;">金符文</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="recipe-tab-silver" data-category="silver-rune-category" style="padding: 8px 15px; display: block; text-decoration: none; white-space: nowrap;">银符文</a></li>' +
                '<li style="flex: 1; text-align: center; margin: 0;"><a href="#" class="recipe-tab-bronze" data-category="bronze-rune-category" style="padding: 8px 15px; display: block; text-decoration: none; white-space: nowrap;">铜符文</a></li>' +
                '</ul>';
        ...
        // 通用的过滤函数（基于 option，支持符文分组的特殊显示）
        function filterRecipes(categoryName) {
            // 保存当前选中的分类到全局变量（所有选择框共享）
            globalRuneCategory = categoryName;
            // 同时保存到当前下拉框的data中（用于当前下拉框的状态恢复）
            $dropdown.data('current-category', categoryName);
            ...
            if (categoryName === 'gold-rune-category') {
                ...
                runeList = silverRunes;
            } else if (categoryName === 'bronze-rune-category') {
                runeList = bronzeRunes;
            }
            
            // 按符文分组 + 时间排序
            visibles.sort(function(a, b) {
                var aRuneName = $(a).attr('data-rune-name') || '';
                var bRuneName = $(b).attr('data-rune-name') || '';
                
                // 先按符文分组排序
                var aRuneIndex = runeList.indexOf(aRuneName);
                var bRuneIndex = runeList.indexOf(bRuneName);
                
                if (aRuneIndex !== bRuneIndex) {
                    // 如果符文不同，按符文顺序排序
                    if (aRuneIndex === -1) return 1;  // a 不在列表中，排到后面
                    if (bRuneIndex === -1) return -1; // b 不在列表中，排到后面
                    return aRuneIndex - bRuneIndex;
                }
            ...
            // 4) 符文分组和冲突检测的特殊处理（在 refresh 后操作 li 元素）
            // 重新获取 $innerMenu，因为 refresh 可能重建了 DOM
            $innerMenu = sp && sp.$menuInner ? sp.$menuInner : $dropdown.find('ul.inner');
            
            // 移除之前创建的符文分组标题
            $innerMenu.find('li.rune-group-header').remove();
            $innerMenu.find('li.rune-conflict-tabs').remove();
            
            // 按符文分组收集菜谱（从 li 元素中收集，因为 refresh 后 li 已经根据 option 重建）
            var recipesByRune = {};
            ...
            $innerMenu.find('li').each(function() {
                ...
                // 从对应的 option 获取数据
                var liIndex = $innerMenu.find('li:not(.divider):not(.disabled):not(.hidden)').index($li);
                var $correspondingOption = $select.find('option:not([hidden])').eq(liIndex);
                
                if ($correspondingOption.length > 0) {
                    var runeName = $correspondingOption.attr('data-rune-name') || '';
                    var guestNames = $correspondingOption.attr('data-guest-names') || '';
                    
                    // 将数据同步到 li 元素
                    if (runeName) $li.attr('data-rune-name', runeName);
                    if (guestNames) $li.attr('data-guest-names', guestNames);
                    
                    if (runeName && recipeLisByRune[runeName]) {
                        recipeLisByRune[runeName].push($li);
                        recipesByRune[runeName].push($li);
                    }
                }
            });
            
            // ========== 冲突检测逻辑 ==========
            // 参考show项目的实现，按贵客分组检测冲突
            var gameData = calCustomRule.gameData || null;
                            ...
                            for (var r in rule.custom[c].recipes) {
                                if (rule.custom[c].recipes[r].data) {
                                    selectedRecipesOnField.push({
                                        recipeName: rule.custom[c].recipes[r].data.name,
            ...
            // 当前选择框已经选中的菜谱名称（用于排除“跟自己冲突”的情况）
            // 当前格子真正已上场的菜名：直接从 calCustomRule 中拿，更可靠
            var currentSelectedRecipeName = "";
            try {
                var currentRuleIndex = $(".cal-custom-item").index($select.closest(".cal-custom-item"));
                var currentItemIndex = $select.closest(".cal-custom-item").find(".selected-item").index($select.closest(".selected-item"));
                var currentRecipeIndex = $select.closest(".selected-item").find(".recipe-box").index($select.closest(".recipe-box"));
                
                if (calCustomRule && calCustomRule.rules &&
                    calCustomRule.rules[currentRuleIndex] &&
                    calCustomRule.rules[currentRuleIndex].custom &&
                    calCustomRule.rules[currentRuleIndex].custom[currentItemIndex] &&
                    calCustomRule.rules[currentRuleIndex].custom[currentItemIndex].recipes &&
                    calCustomRule.rules[currentRuleIndex].custom[currentItemIndex].recipes[currentRecipeIndex] &&
                    calCustomRule.rules[currentRuleIndex].custom[currentItemIndex].recipes[currentRecipeIndex].data) {
                    
                    currentSelectedRecipeName = (calCustomRule.rules[currentRuleIndex]
                        .custom[currentItemIndex]
            ...
            runeList.forEach(function(runeName) {
                ...
                recipes.forEach(function($li) {
                    var recipeName = $li.find('.name').text().trim();
                            ...
                            if (guest.gifts) {
                                guest.gifts.forEach(function(gift) {
                                    if (gift.recipe === recipeName && gift.antique === runeName) {
                                        ...
                                        if (!alreadyAdded) {
                                            recipesByGuest[guest.name].push({
                                                $li: $li,
                                                recipeName: recipeName,
                                                time: parseInt($li.find('.score').text()) || 0
                                            });
                                        }
                                    }
                                });
            ...
            // 确保 ul.inner 恢复正常布局（不使用 flexbox）
            $innerMenu.css({display: '', 'flex-direction': ''});
            
            // 收集所有菜谱的贵客信息，用于检测冲突并标记
            var allConflictGuests = [];
            ...
            // 遍历所有符文分组，收集贵客信息
            runeList.forEach(function(runeName) {
                var runeData = recipesByRune[runeName];
                if (!runeData || !runeData.all || runeData.all.length === 0) return;
                
                var recipes = runeData.all;
                var guestToRecipesInRune = {};  // 当前符文内的贵客 -> 菜谱映射
                
                // 收集当前符文分类中所有菜谱的贵客信息
                recipes.forEach(function($recipe) {
                    var guestNames = $recipe.attr('data-guest-names');
                    var recipeName = $recipe.find('a .name').text().trim();
                    if (guestNames) {
                        // 处理格式：符文名-贵客1、贵客2
                        var guestsOnly = guestNames;
                        if (guestNames.indexOf('-') !== -1) {
                            // 去掉符文名称部分
                            var parts = guestNames.split('-');
                            guestsOnly = parts.slice(1).join('-'); // 处理贵客名称中可能包含 '-' 的情况
                        }
                        
                        var guestArray = guestsOnly.split('、');
                        guestArray.forEach(function(guestName) {
                            // 全局映射
            ...
            // 调试：输出冲突贵客列表
            console.log('=== 贵客冲突检测开始 ===');
            console.log('同一符文内冲突:', sameRuneConflictGuests);
            console.log('跨符文冲突:', crossRuneConflictGuests);
            console.log('贵客到菜谱映射:', guestToRecipesMap);
            console.log('贵客到符文映射:', guestToRunesMap);
            console.log('=== 贵客冲突检测结束 ===');
            
            // 找出与场上菜谱有冲突的贵客
            var fieldConflictGuests = [];
            if (gameData && gameData.guests && selectedRecipesOnField.length > 0) {
                gameData.guests.forEach(function(guest) {
                    if (guest.gifts) {
                        guest.gifts.forEach(function(gift) {
                            for (var i = 0; i < selectedRecipesOnField.length; i++) {
                                if (gift.recipe === selectedRecipesOnField[i].recipeName) {
            ...
            runeList.forEach(function(runeName) {
                ...
                recipes.forEach(function($recipe) {
                    // 在符文分类中，修改菜谱显示内容，添加贵客信息
                    var guestNames = $recipe.attr('data-guest-names');
                    if (guestNames) {
                        var $link = $recipe.find('a');
                        var $name = $link.find('.name');
                        if ($name.length > 0) {
                            // 保存原始名称（如果还没保存）
                            if (!$name.attr('data-original-text')) {
                                $name.attr('data-original-text', $name.text());
                            }
                            
                            // 获取菜谱名称
                            var recipeName = $name.attr('data-original-text');
                            
                            // 检查是否包含符文信息（格式：符文名-贵客名）
                            var displayText = guestNames;
                            var guestArray = [];
                            ...
                            if (guestNames.indexOf('-') !== -1) {
                                // 提取符文名称和贵客名称
                                var parts = guestNames.split('-');
                                var currentRuneName = parts[0];
                                var guestsOnly = parts.slice(1).join('-'); // 处理贵客名称中可能包含 '-' 的情况
                                
                                // 分割贵客名称
                                guestArray = guestsOnly.split('、');
                                
                                // 检查每个贵客是否有冲突，并应用不同颜色
                                var guestHtml = guestArray.map(function(guestName) {
                                    // 检查是否与场上菜谱冲突（优先级最高，红色）
                                    if (fieldConflictGuests.indexOf(guestName) !== -1) {
                                        console.log('贵客与场上冲突（红色）:', guestName);
                                        return '<span style="color: red; font-weight: bold;">' + guestName + '</span>';
                                    }
                                    
                                    // 检查是否在同一符文内有冲突（红色）
                                    var hasSameRuneConflict = sameRuneConflictGuests[currentRuneName] && 
                                                             sameRuneConflictGuests[currentRuneName].indexOf(guestName) !== -1;
                                    if (hasSameRuneConflict) {
                                        console.log('贵客同符文内冲突（红色）:', guestName, '符文:', currentRuneName);
                                        return '<span style="color: red;">' + guestName + '</span>';
                                    }
                                    
                                    // 检查是否跨符文冲突（黄色）
                                    var hasCrossRuneConflict = crossRuneConflictGuests.indexOf(guestName) !== -1;
                                    if (hasCrossRuneConflict) {
                                        console.log('贵客跨符文冲突（黄色）:', guestName);
                                ...
                                // 组合显示：符文名-贵客名
                                displayText = currentRuneName + '-' + guestHtml;
                            } else {
                                // 没有符文信息，直接处理贵客名称
                                guestArray = guestNames.split('、');
                                var guestHtml = guestArray.map(function(guestName) {
                                    // 检查是否与场上菜谱冲突
                                    if (fieldConflictGuests.indexOf(guestName) !== -1) {
                                        return '<span style="color: red; font-weight: bold;">' + guestName + '</span>';
                                ...
                                displayText = guestHtml;
                            }
                            
                            // 修改显示：菜谱名 + 符文和贵客信息在同一行，其他信息换行
                            $name.html(recipeName + ' <span style="font-size: 12px; color: #666;">(' + displayText + ')</span>');
                        }
                    }
                    
                    // 标记菜谱属于哪个符文分组（用于搜索等功能）
                    $recipe.addClass('rune-group-item')
                        .attr('data-rune-group', runeName);
                });
            });
        }
        
        // 绑定标签点击事件
        $dropdown.find('.recipe-tab-all, .recipe-tab-gold, .recipe-tab-silver, .recipe-tab-bronze').off('click').on('click', function(e) {
            e.stopPropagation();
            e.preventDefault();
            $(this).parent().addClass('active').siblings().removeClass('active');
            ...
            // 切分类：先按分类/排序重建列表，再让搜索在当前分类结果里生效
            var searchText = ($dropdown.find('.bs-searchbox input').val() || '').trim();
            filterRecipes(categoryName === 'all' ? null : categoryName);
            // refresh 会重建 searchbox，需要重新获取后再触发搜索
            var $newSearchInput = $dropdown.find('.bs-searchbox input');
            if (searchText && $newSearchInput.length > 0) {
                $newSearchInput.val(searchText).trigger('input').trigger('keyup');
        ...
        // 立即应用保存的分类，避免闪烁
        var categoryToApply = globalRuneCategory !== null ? globalRuneCategory : $dropdown.data('current-category');
        
        if (categoryToApply !== undefined) {
            // 恢复标签选中状态
            $dropdown.find('.recipe-category-tabs li').removeClass('active');
            
            if (categoryToApply === null) {
                // 显示"全部"标签
                $dropdown.find('.recipe-tab-all').parent().addClass('active');
                filterRecipes(null);
            } else {
                // 显示对应的符文分类标签
                var $targetTab = $dropdown.find('.recipe-category-tabs a[data-category="' + categoryToApply + '"]');
                if ($targetTab.length) {
                    $targetTab.parent().addClass('active');
                    filterRecipes(categoryToApply);
                } else {
                    // 如果没有找到对应的标签，显示全部
                    $dropdown.find('.recipe-tab-all').parent().addClass('active');
                    filterRecipes(null);
        ...
        // 自定义搜索功能：只隐藏/显示菜谱，不改变UI结构
        var $searchInput = $dropdown.find('.bs-searchbox input');
        if ($searchInput.length > 0) {
            // 阻止Bootstrap-select的默认搜索行为
            $searchInput.off('input propertychange');
            
            // 实现自定义搜索
            $searchInput.on('input propertychange', function() {
                var searchText = $(this).val().trim().toLowerCase();
                var currentCategory = $dropdown.data('current-category');
                ...
                if (currentCategory) {
                    ...
                    if (searchText) {
                        // 有搜索内容：遍历所有菜谱li，根据菜名匹配来显示/隐藏
                        $innerMenu.find('li').each(function() {
                            var $li = $(this);
                            
                            // 跳过divider和disabled
                            if ($li.hasClass('divider') || $li.hasClass('disabled')) {
                                return; // continue
                            }
                            
                            // 获取菜谱名称
                            var recipeName = $li.find('a .name').text().toLowerCase();
                            
                            // 判断是否匹配搜索文本
                            if (recipeName.indexOf(searchText) >= 0) {
                                // 匹配：显示
                                $li.css('display', '');
                            } else {
                                // 不匹配：隐藏
    ...
    $("select.select-picker-recipe").on("changed.bs.select", function(a, t, i, l) {
        ...
        // 默认使用 select 的值（recipeId）
        var n = $(this).val();
        
        // 贵客率模式下，为了避免下拉样式分组/隐藏导致索引错位，
        // 额外根据"实际点击的 li 的菜名"反查一次 recipeId，保证选择的是用户点的那道菜
        if (isGuestRateMode() && typeof t === "number" && t >= 0) {
            try {
                var sp = $(this).data("selectpicker");
                var $dropdown = sp && sp.$menuInner ? sp.$menuInner : $(this).parent().find(".dropdown-menu ul.inner");
                // 获取真正的选项元素（跳过disabled和divider）
                var $allLis = $dropdown.find("li");
                var $validLis = $allLis.filter(function() {
                    return !$(this).hasClass("disabled") && 
                           !$(this).hasClass("divider");
                });
                var $li = $validLis.eq(t);
                var clickedName = $.trim($li.find(".name").text() || $li.text());
                if (clickedName) {
                    var rule = calCustomRule.rules[s];
                    if (rule && rule.menus) {
                        var fixedId = getCustomRecipeIdByName(rule.menus, clickedName);
                        if (fixedId) {
                            n = fixedId;
                        }
                    }
        ...
        // 贵客率计算模式：选择菜谱后检测贵客冲突
        if (isGuestRateMode()) {
            checkGuestConflictsForSelectedRecipes(s);
        }
    }),
...
function getOriginVal(e) {
    var a = 0;
    return "菜棚" == e ? a = 1 : "菜地" == e ? a = 2 : "森林" == e ? a = 3 : "鸡舍" == e ? a = 4 : "猪圈" == e ? a = 5 : "牧场" == e ? a = 6 : "作坊" == e ? a = 7 : "池塘" == e && (a = 8),
    a
}
...
function setPartialUltimateOptions(e) {
    ...
    timeTypeChefs.sort(function(e, a) {
        ...
        if (e.chef.equip && e.chef.equip.effect) {
            // 处理MutiEquipmentSkill类技能（使厨具技能效果翻倍）
            // 使用 e.effect（修炼技能）而不是 e.chef.selfUltimateEffect
            var eEquipEffect = updateEquipmentEffect(e.chef.equip.effect, e.effect || []);
        ...
        if (a.chef.equip && a.chef.equip.effect) {
            // 处理MutiEquipmentSkill类技能（使厨具技能效果翻倍）
            // 使用 a.effect（修炼技能）而不是 a.chef.selfUltimateEffect
            var aEquipEffect = updateEquipmentEffect(a.chef.equip.effect, a.effect || []);
                ...
                if (localData && localData.chefs) {
                    for (var lci = 0; lci < localData.chefs.length; lci++) {
                        if (localData.chefs[lci].id == e[t].chefId && "是" == localData.chefs[lci].ult) {
                            ultimatedChefIds.push(e[t].chefId);
                            break;
                        }
                    }
                }
...
function getOptionsString(e) {
    ...
    return $.each(e, function(e, t) {
        ...
        t.runeType && (a += " data-rune-type='" + t.runeType + "'"),
        t.runeName && (a += " data-rune-name='" + t.runeName + "'"),
        t.guestNames && (a += " data-guest-names='" + t.guestNames + "'"),
        t.skillValues && (a += " data-skill-values='" + JSON.stringify(t.skillValues) + "'"),
...
var calCustomRule, private = !1, isMobile = !1, expendBtn = !0, runeGroupExpandState = {}, globalRuneCategory = null, globalChefCategory = null, globalEquipCategory = null;
