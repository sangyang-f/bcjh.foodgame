...
function calCustomResults(e) {
    ...
    for (var a = 0, t = 0; t < calCustomRule.rules.length; t++) {
        ...
        for (var c in l) {
            ...
            // 李清凝调试日志
            if (l[c].chef.name === '李清凝') {
                if (s[c] && s[c].length > 0) {
                    for (var si = 0; si < s[c].length; si++) {
                    }
                }
                // 打印遗玉信息
                if (l[c].chef.disk && l[c].chef.disk.ambers) {
                    for (var ai = 0; ai < l[c].chef.disk.ambers.length; ai++) {
                        if (l[c].chef.disk.ambers[ai].data) {
                        }
                    }
                }
            }
...
function getCustomRecipesOptions(e, a, t, i) {
                ...
                if (u.hasOwnProperty("MaterialsNum")) {
                    ...
                    if (isGuestRateModeFlag) {
                        ...
                        if (isUnultimatedMode && isGodModeFlag) {
                            // 神级方案模式：不显示时间，只显示技法值（技法值已在skillDisp中）
                            w.order = 0;
    ...
    if (isUnultimatedMode && isQuestCategory) {
        // 修炼任务分类：过滤并排序
        var chef = h[a].chef;
...
function initCalCustomTable(e) {
    ...
    $(".equip-box").click(function() {
        ...
        // 先解绑旧的事件，避免重复绑定
        $equipSelect.off("changed.bs.select").off("change");
        
        $equipSelect.html(getOptionsString(l)).selectpicker("refresh").selectpicker("toggle");
        
        // 重新绑定事件 - 使用多种方式确保事件能触发
        $equipSelect.on("changed.bs.select change", function(evt, clickedIndex, isSelected, previousValue) {
        ...
        $equipSelect.on("hidden.bs.select", function(evt) {
            var ruleIndex = $(".cal-custom-item").index($(this).closest(".cal-custom-item"));
            var chefIndex = $(this).closest(".cal-custom-item").find(".selected-item").index($(this).closest(".selected-item"));
            var equipId = $(this).val();
            if (equipId) {
        ...
        // 额外监听下拉菜单项的点击事件
        var sp = $equipSelect.data('selectpicker');
        if (sp && sp.$menu) {
            // 绑定到 .inner 内的列表项
            sp.$menu.find('.dropdown-menu.inner').off('click.equipSelect').on('click.equipSelect', 'li', function(evt) {
                // 忽略分类标签的点击
                ...
                if (index !== undefined && index !== null) {
                    var $option = $equipSelect.find('option').eq(parseInt(index));
                    if ($option.length) {
                ...
                if (selectedValue === null) {
                    ...
                    $equipSelect.find('option').each(function() {
                ...
                if (selectedValue !== null && selectedValue !== undefined) {
                    var ruleIndex = $(".cal-custom-item").index($equipSelect.closest(".cal-custom-item"));
                    var chefIndex = $equipSelect.closest(".cal-custom-item").find(".selected-item").index($equipSelect.closest(".selected-item"));
    ...
    // 使用事件委托监听 hidden.bs.select 事件作为备用
    $(document).on("hidden.bs.select", "select.select-picker-equip", function(a) {
        var s = $(".cal-custom-item").index($(this).closest(".cal-custom-item"))
          , c = $(this).closest(".cal-custom-item").find(".selected-item").index($(this).closest(".selected-item"))
          , r = $(this).val();
        if (r) {
