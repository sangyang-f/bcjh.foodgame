...
function clearAllSelectedOnField(gameData) {
    ...
    console.log("【清空场上已选】已清空所有厨师、厨具、调料、菜谱");
}
        ...
        l)
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    ...
                    // 修炼查询模式：显示神差值；普通模式：显示可制作差值
                    var isUnultimatedMode = $("#chk-unultimated-mode").prop("checked");
                    if (isUnultimatedMode) {
                        // 修炼模式：始终显示神级差值
                        var k = getSkillDiff(l[c].chef, l[c].recipes[r].data, 4);
                        if (k.value > 0) {
                            l[c].recipes[r].skillDiff = "神差值：" + k.disp;
                        } else {
...
function getCustomChefsOptions(e, a, t) {
    ...
    var isUnultimatedModeForFilter = $("#chk-unultimated-mode").prop("checked");
    ...
    for (var h in f) {
        ...
        if (!i || isOwned) {
            ...
            // 检查是否为修炼查询模式
            isUnultimatedMode = $("#chk-unultimated-mode").prop("checked"),
                    ...
                    for (var v in h[b].recipes)
                        if (h[b].recipes[v].data) {
                            ...
                            b == a && v == t) {
                                ...
                                var isUnultimatedMode = $("#chk-unultimated-mode").prop("checked");
                                if (isUnultimatedMode) {
                                    var O = getSkillDiff(h[a].chef, x.data, 4);
                                    if (O.value > 0) {
                                        A = "神差值：" + O.disp;
                                    } else {
                                        A = "";
