        ...
        if (!e.rules[t].Hide && e.rules[t].Id) {
            ...
            if (!(i && s < i || l && s > l)) {
                ...
                $("#select-cal-rule optgroup").each(function() {
                    c.indexOf($(this).attr("label")) >= 0 && ($(this).append(r),
                    n = !0)
                }),
...
function loadCalRule(e) {
    var a = Math.floor($("#select-cal-rule").val());
...
function setCalConfigData(e) {
    for (var a in calCustomRule.rules) {
        ...
        for (var R in t.menus) {
            ...
            var G = getRecipeQuantity(t.menus[R].recipe.data, t.materials, t, null)
              , P = getRecipeResult(null, null, t.menus[R].recipe.data, G, G, t.materials, t, t.decorationEffect, null, !1, buildRecipeMenu(t.menus[R].recipe.data), null, null);
            P.available = G,
...
function calCustomResults(e) {
    for (var a = 0, t = 0; t < calCustomRule.rules.length; t++) {
                ...
                if (l[c].recipes[r].data)
                    for (var n in i.menus)
                        if (i.menus[n].recipe.data.recipeId == l[c].recipes[r].data.recipeId) {
                            ...
                            l[c].recipes[r].max = i.menus[n].recipe.max;
                            break
                        }
            ...
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    var b = $(".cal-custom-item:eq(" + t + ") .selected-item:eq(" + c + ") .recipe-box:eq(" + r + ") .recipe-condiment input").prop("checked")
                      , g = getRecipeResult(l[c].chef, l[c].equip, l[c].recipes[r].data, l[c].recipes[r].quantity, l[c].recipes[r].max, i.materials, i, i.decorationEffect, b ? l[c].condiment : null, !0, l[c].recipes, o[3 * c + Number(r)], p[3 * c + Number(r)]);
                    if (l[c].recipes[r] = g,
                    f += g.totalTime,
                    d += g.totalBonusScore,
        ...
        for (var c in l)
            for (var r in l[c].recipes)
                if (l[c].recipes[r].data) {
                    ...
                    w.find(".max").html(D),
                    i.hasOwnProperty("MaterialsNum") ? w.find(".available").html(0 != x ? "(" + x + ")" : "") : w.find(".available").html(C < 0 ? "(" + C + ")" : "")
                }
        ...
        if (i.hasRuleAddition && !i.hasOwnProperty("scoreMultiply") && (_ += " 规则分：" + d),
        _ += " 总分：" + u,
...
function getCalRecipeBox2Disp(e, a, t) {
    ...
    e.bonusAdditionDisp ? i += "<span>规则" + e.bonusAdditionDisp + "</span>" : i += "<span class='blank'>规则</span>",
    ...
    for (var y in k) {
        ...
        if ((!l || x.data.got) && !(s.length > 0 && s.indexOf(x.data.rarity.toString()) < 0)) {
            ...
            if (!(n.length > 0 && n.indexOf(x.data.condiment) < 0)) {
                ...
                var T = getRecipeQuantity(x.data, m, u, h[a].chef)
...
function updateCalMenus() {
    ...
    for (var t in e.menus) {
        var i = getRecipeQuantity(e.menus[t].recipe.data, e.rest, e, null);
        for (var l in a)
